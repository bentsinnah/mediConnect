...
function notifIcon(title: string) {
  ...
  if (title.includes('Appointment') || title.includes('📅') || title.includes('✅')) return <Calendar size={18} />;
      ...
      <header className="section-header" style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            ...
            {unreadCount > 0 && <p style={{ fontSize: '0.8rem', color: 'var(--blue-primary)', fontWeight: 600 }}>{unreadCount} unread</p>}
          </div>
        </div>
        {unreadCount > 0 && (
          ...
          {notifications.map((n: any) => (
            <div 
              ...
              style={{ 
                padding: 16, 
                borderRadius: 16, 
                border: `1px solid ${!n.isRead ? 'var(--blue-primary)' : 'var(--border-color)'}`,
                background: !n.isRead ? 'rgba(59, 130, 246, 0.04)' : 'white',
                display: 'flex',
                gap: 12,
            ...
            >
              <div style={{ 
                ...
                background: !n.isRead ? 'var(--blue-primary)' : 'var(--bg-page)', 
              ...
              {!n.isRead && <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--blue-primary)', marginTop: 6, flexShrink: 0 }} />}
            </div>
          ))}
        </div>
      )}
